...
<%
    ...
    try {
        ...
        String sqlCheck = "SELECT * FROM C##ESTATE.ELEC_MASTER WHERE SECTOR = ? AND PLOT = ?";
        ...
        if (rs.next()) {
            ...
            rs.close();
            stmt.close();
            String sqlTPMas = "SELECT SECTOR, PLOT, SECTOR || '/' || PLOT AS CONSUMER_NO, LOCATION, NAME, ELEC_CODE, STD_LUNITS, STD_PUNITS, LOAD_KW, STATUS, DECODE(SECTOR, 'AB', NULL, TRADE) AS TRADE, SUBSTR(NATURE, 1, 4) AS NATURE FROM C##ESTATE.TP_MAS WHERE SECTOR = ? AND PLOT = ?";
            stmt = conn.prepareStatement(sqlTPMas);
            stmt.setString(1, sector);
            stmt.setString(2, plot);
            rs = stmt.executeQuery();
